									  ...
									  const sqbind::stdString &x_sData )
{
	...
	mImg[ "type" ].set( sType.Ptr() );
	mImg[ "width" ].set( oexMks( img.GetWidth() ).Ptr() );
	mImg[ "height" ].set( oexMks( img.GetHeight() ).Ptr() );
	mImg[ "img" ].set( sqbind::stdString().assign( (char*)pBuf, nSize ) );
