...
const unsigned char CMd5::PADDING[64] = {
...
void CMd5::MD5(unsigned char digest[16], const void *input, unsigned int inputLen)
{
	MD5_CTX ctx;
	MD5Init( &ctx );
...
bool CMd5::vHash( unsigned char digest[16], unsigned int uBuffers, void **pArgs )
{
	MD5_CTX ctx;
	...
	void **ptrExtra = pArgs;
	for ( unsigned int i = 0; i < uBuffers; i++ )
	{	OEXLIB_TRY 
		{	
			// Get pointers
			unsigned char *pPtr = *(unsigned char **)( ptrExtra );
