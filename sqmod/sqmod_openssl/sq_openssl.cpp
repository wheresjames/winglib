...
int CSqOpenSSL::SignBin( COsslKey *pKey, sqbind::CSqBinary *pData, sqbind::CSqBinary *sig )
{_STT();

	if ( !pKey || !pData || !sig || !pKey->getPrivateKey() )
		return 0;
...
int CSqOpenSSL::VerifyBin( COsslKey *pKey, sqbind::CSqBinary *pData, sqbind::CSqBinary *sig )
{_STT();

	if ( !pKey || !pData || !sig || !pKey->getPublicKey() )
		return 0;
