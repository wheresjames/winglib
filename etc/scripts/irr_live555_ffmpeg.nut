...
/*
		...
		if ( !_g.tex )
		{
			...
			_g.dec.Decode( frame, CFfFmt().AV_PIX_FMT_RGB32, CSqBinary(), CSqMulti() );
...
*/			local tex = _g.tex.Lock();
			if ( tex.getUsed() )
			{
				...
				if ( _g.ffmpeg.getVideoStream() != _g.ffmpeg.ReadFrame( _g.frame, inf ) )
					...
					if ( !_g.ffmpeg.DecodeFrameBin( _g.frame, CFfFmt().AV_PIX_FMT_RGB32, tex, inf ) )
