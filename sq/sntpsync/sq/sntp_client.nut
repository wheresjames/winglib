...
function _init() { return 0; }
function _idle() { return 0; }

function SyncTime( server, port )
{
	port = ::_self.tolong( port );
...
//	dw0 = dw0 | ( ( 0 & 0xff ) << 0 );	// Precision
	...
	// Correct time if off by even a second
	if ( ts_client != ts_server )
	{	::_self.echo( "Correcting time" );	
		tm.SetSystemTime();
	} // end if
